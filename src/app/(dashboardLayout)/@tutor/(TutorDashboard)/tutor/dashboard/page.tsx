...
const TutorDashboard = () => {
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="rounded-[32px] bg-gradient-to-br from-sky-600 via-cyan-600 to-teal-500 p-8 text-white shadow-xl shadow-sky-100">
        <div className="flex items-start gap-5">
          ...
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm font-medium text-sky-50/80">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: "Slots", val: profile?.availability?.length ?? 0, icon: CalendarDays, color: "text-sky-600", bg: "bg-sky-50" },
          { label: "Upcoming", val: confirmedCount, icon: Video, color: "text-sky-600", bg: "bg-sky-50" },
          { label: "Completed", val: completedCount, icon: ShieldCheck, color: "text-emerald-600", bg: "bg-emerald-50" },
          { label: "Rating", val: profile?.avgRating ? profile.avgRating.toFixed(1) : "0.0", icon: Star, color: "text-amber-500", bg: "bg-amber-50", fill: true },
        ].map((stat, i) => (
          <div key={i} className="rounded-[28px] border border-slate-100 bg-white p-6 shadow-sm transition-all hover:shadow-md">
        ...
        <div className="space-y-6">
          <div className="rounded-[32px] border border-slate-100 bg-white p-8 shadow-sm">
            <div className="flex items-center justify-between mb-8">
              ...
              <Button asChild variant="ghost" className="text-sky-600 font-bold hover:bg-sky-50 rounded-xl">
                <Link href="/tutor/sessions">View all</Link>
              </Button>
            </div>
            
            {nextSession ? (
              <div className="rounded-2xl bg-slate-50/50 p-6 border border-slate-100 transition-colors hover:bg-slate-50">
                ...
                <div className="flex items-center gap-2 mt-4 text-sm font-bold text-slate-600 bg-white w-fit px-3 py-1.5 rounded-lg border border-slate-100 shadow-sm">
                  <CalendarDays className="h-4 w-4 text-sky-500" />
                  {new Date(nextSession.scheduledAt).toLocaleString()}
                </div>
              </div>
            ) : (
              <div className="text-center py-10 rounded-2xl border-2 border-dashed border-slate-100 text-slate-400 font-medium">
        ...
        <div className="space-y-6">
          <div className="rounded-[32px] border border-slate-100 bg-white p-8 shadow-sm">
            ...
            <div className="grid gap-3">
              <Button asChild className="h-12 rounded-xl w-full bg-sky-600 font-bold text-white hover:bg-sky-700 shadow-lg shadow-sky-100 transition-all active:scale-[0.98]">
                <Link href="/tutor/profile">Update Profile</Link>
              </Button>
              <Button asChild variant="outline" className="h-12 rounded-xl w-full border-slate-100 font-bold text-slate-600 hover:bg-slate-50">
            ...
            <div className="space-y-5">
              ...
              <div className="pt-5 border-t border-slate-200">
                <div className="flex items-center justify-between">
                  ...
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 px-2.5 py-1 text-[10px] font-black text-emerald-700 uppercase">
